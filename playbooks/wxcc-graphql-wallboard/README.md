...
You run an **Express** server that serves static wallboard pages from `src/views` and JSON endpoints for each chart. When a chart loads, the server resolves **org ID** and **access token**: in the default **dev** path, `controller/decide.js` reads `ENVIRONMENT` and `ORG_ID` from `/environment`, then the token helper calls **`/dev-token`** with **Basic auth** (`NAME` / `PASS` from the environment). That route returns **`DEV_TOKEN`** from the environment (so the token is never embedded in client-side code). Wallboard controllers then **POST** GraphQL queries to **`{WXCC_API_BASE}/search`** with your org ID and `Authorization: Bearer <token>`.
...
- **`controller/decide.js`** — Chooses dev vs production token source; calls this app’s `/environment` using `URL` from the environment.
...
This is **sample code**, not a production integration: simplified token handling, fixed CORS allow list, and hard-coded query `from` epochs that you must refresh over time. See [docs/upstream-overview.md](docs/upstream-overview.md) for upstream video, demo link, and query window notes.
...
5. **Start** the server: `npm run dev` (nodemon) or `npm start` (plain `node server.js`).
...
7. **If charts are empty or error:** Confirm the token is valid, `WXCC_API_BASE` matches your region, and update the `from` epoch in each `controller/wallboard/*.js` query so the time window overlaps data in your tenant (see [docs/upstream-overview.md](docs/upstream-overview.md)).
...
- **Query windows:** GraphQL `from` values in the wallboard files are fixed millisecond epochs; they must be updated for meaningful results as time passes (see [docs/upstream-overview.md](docs/upstream-overview.md)).
...
- **`URL` vs listen address:** `decide.js` requests `/environment` on the base URL in `URL`; set `URL` to match where the app is reachable (host and port).
