#!/usr/bin/env python
"""
UCM to Webex Calling — Bulk User Provisioning Script
=====================================================
This script reads users from Cisco UCM via the AXL SOAP API and provisions
them in Webex Calling using the wxc_sdk async Python SDK.

What it does:
  - Validates the Webex access token
  - Reads all UCM users via the ucm_reader module (AXL)
  - Filters users to those with a valid mailid, primaryExtension, and +E.164 telephoneNumber
  - Groups users by NPA, then focuses on a target NPA (default: +1408)
  - For each user, checks for TN and extension availability in Webex Calling
  - Creates a Webex user and assigns a calling license, extension, and DID

What it does NOT do:
  - It is not production-hardened (no retry logic, limited error handling)
  - It does not migrate voicemail, call forwarding, or hunt groups
  - It does not import phone numbers — numbers must be pre-loaded in Webex Calling
  - The webex_email() function generates synthetic Gmail plus-addresses for testing;
    replace it with real user email lookup for production use

Required environment variables (set in .env or shell):
  AXL_HOST           — UCM publisher hostname or IP
  AXL_USER           — AXL-enabled UCM user
  AXL_PASSWORD       — AXL user password
  WEBEX_ACCESS_TOKEN — Personal access token from developer.webex.com (~12 hour TTL)
  GMAIL_ID           — Gmail address used to construct synthetic test email addresses

SECURITY NOTES:
  - READONLY = True by default. Set to False only when ready to provision real users.
  - This script logs user PII (email addresses, phone numbers, display names) to both
    the console and main.log. Implement log redaction before using in production.
  - The Webex personal access token expires in ~12 hours. Re-obtain it from
    developer.webex.com if the migration runs longer than this.
"""
import asyncio
import datetime
import logging
...
# SAFETY: Set READONLY = False only when you have completed a successful dry run
# and are ready to create real users in Webex Calling. When True, all provisioning
# API calls are skipped — validation and logging still run.
READONLY = True
...
async def user_provisioning(*, users: List[User]):
    ...
    async def provision_single_user(location: Location,
        ...
        :return:
        """
        # SECURITY NOTE: user.mailid, user.telephoneNumber, user.firstName, and user.lastName
        # are PII. They are logged below to console and main.log. Implement log redaction
        # (e.g. mask email localpart, truncate phone number) for production deployments.
...
if __name__ == '__main__':
    # SECURITY NOTE: The handlers below log PII (user emails, phone numbers, names) to console
    # and to main.log on disk. For production deployments, apply log redaction filters or replace
    # these handlers with a structured logging solution that masks sensitive fields.
