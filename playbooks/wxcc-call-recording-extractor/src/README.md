...
**Maven:** Run every Maven command from **this folder** (the one that contains `pom.xml`). You must pass a lifecycle phase such as `package` or `install`. Running **`mvn` with no arguments** fails with `No goals have been specified`. This Java sample is **not** the Node-based `call-recording-download-sample` playbook; that one uses `npm`, not Maven.
...
- In this Playbook, the Maven project root is `playbooks/wxcc-call-recording-extractor/src`. Upstream sample: [call-recording-extractor-sample](https://github.com/WebexSamples/webex-contact-center-api-samples/tree/main/call-recording-samples/call-recording-extractor-sample).
...
| client_id | value from https://developer.webex.com/create/docs/integrations |
| client_secret | value from https://developer.webex.com/create/docs/integrations |
| redirect_uri | value from https://developer.webex.com/create/docs/integrations |
...
- Build the JAR (example): `mvn clean package -DskipTests` or `mvn clean install -DskipTests`

**Step 4:**

- Run the application locally using: `java -jar target/capture-0.0.1-SNAPSHOT.jar` (path is under the `target` directory created in Step 3)
...
- [My Apps](https://developer.webex.com/create/docs/integrations)
...
- [GraphQL APIs Search Tasks](https://developer.webex.com/webex-contact-center/docs/getting-started-with-search-api)
