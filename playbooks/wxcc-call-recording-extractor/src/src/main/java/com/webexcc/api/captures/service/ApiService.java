...
public class ApiService {
	...
	/** WxCC agents/activities rejects windows older than ~12 months or with {@code to} in the future. */
	private static final long TWELVE_MONTHS_MS = 366L * 24 * 60 * 60 * 1000;
	...
	private AgentsActivities getAgentsActivitiesByFromTo(String channelTypes, long from, long to) throws Exception { 
		...
		long oldestAllowed = now - TWELVE_MONTHS_MS;
		// API returns 400 if "to" is in the future or the window is outside retention.
		to = Math.min(to, now);
		if (to <= oldestAllowed) {
			logger.debug("Skipping agents/activities: entire window before 12-month retention (to={})", to);
			return new AgentsActivities();
		}
		from = Math.max(from, oldestAllowed);
		if (from >= to) {
			logger.debug("Skipping agents/activities: empty window after clamp (from={} to={})", from, to);
			return new AgentsActivities();
		}
