/**
 * Webex Contact Center token helper for Google Cloud Functions + Firestore.
 *
 * Exposes HTTP routes to complete OAuth (callback), initialize a token doc (/init),
 * and return a current access token (GET with ?name=) using https://webexapis.com/v1/access_token.
 *
 * What this sample does NOT do: production authz beyond a shared passphrase header,
 * rate limiting, audit logging, multi-tenant isolation, or rotation of Firestore-held
 * client secrets. Treat as demonstration code only.
 *
 * Environment (Cloud Function):
 *   TOKEN_PASSPHRASE — shared secret; callers must send header x-token-passphrase with this value.
 *
 * Configuration in Firestore (collection "tokens", document id = token name): clientId,
 * clientSecret, redirectUri, plus tokens written by this function after OAuth/refresh.
 */
...
exports.tokenService = async (req, res) => {
  ...
  if (!expectedPassphrase) {
    return res.status(500).json({ status: "500", message: "Server misconfiguration: TOKEN_PASSPHRASE not set" });
  }
  if (tokenPassphrase !== expectedPassphrase) {
    return res.status(500).json({ status: "500", message: "Invalid Token Passphrase" });
  }
  ...
  try {
    ...
    if (tokenData.accessExpires > currentTime + (2 * 60 * 60 * 1000)) {
      ...
      // Check if Refresh Token has at least two minutes remaining
